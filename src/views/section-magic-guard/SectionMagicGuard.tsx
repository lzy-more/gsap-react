...
function SectionTurbo(): React.ReactElement {
  useGSAP(() => {
    ...
    const timeline = gsap.timeline({
      scrollTrigger: {
        trigger: '.section-magic-guard .section-wrapper', // 触发元素
        start: 'top 65px', // 开始滚动的位置
        pin: true, // 固定触发元素
        scrub: 0.5 // 滚动时动画的同步速度
      }
    });

    // 设置初始状态
    timeline.set('.section-magic-guard .guard-svg', {
      yPercent: 50, // 在Y轴上偏移50%
      scale: 3.6 // 缩放比例为3.6
    });
    timeline.set('.section-magic-guard .guard-path', { strokeWidth: 2 }); // 设置路径的描边宽度为2
    timeline.set('.section-magic-guard .path-icon', { opacity: 0 }); // 设置路径图标的透明度为0

    // 动画：路径线的描边数组变化
    timeline.to('.section-magic-guard .path-line', {
      strokeDasharray: '0% 0% 260%', // 设置描边数组
      duration: 0.5 // 动画持续时间为0.5秒
    });

    // 动画：路径线的颜色变化
    timeline.to('.section-magic-guard .path-line', {
      stroke: '#ffffff', // 设置描边颜色为白色
      duration: 0.5 // 动画持续时间为0.5秒
    });
    // 动画：路径图标的透明度变化，与上一个动画同时开始
    timeline.to(
      '.section-magic-guard .path-icon',
      { opacity: 1, duration: 0.5 }, // 设置透明度为1，动画持续时间为0.5秒
      '<'
    );
