...
// 定义一个名为 SectionTurbo 的函数组件，返回一个 React 元素
function SectionTurbo(): React.ReactElement {
  // 使用 useGSAP 钩子来初始化 GSAP 动画
  useGSAP(() => {
    // 创建一个 GSAP 时间线，并设置滚动触发器
    const timeline = gsap.timeline({
      scrollTrigger: {
        trigger: '.section-turbo .section-wrapper', // 触发动画的元素
        start: 'top 65px', // 动画开始的位置
        pin: true, // 固定触发元素
        scrub: 0.5 // 动画 scrub 效果，使动画与滚动同步
      }
    });

    // 设置初始状态
    timeline.set('.section-turbo .turbo-svg', { yPercent: 50, scale: 3.6 }); // 设置 SVG 的垂直位置和缩放
    timeline.set('.section-turbo .turbo-path', { strokeWidth: 2 }); // 设置路径的描边宽度
    timeline.set('.section-turbo .path-icon', { opacity: 0 }); // 设置路径图标的透明度

    // 动画序列
    timeline.to('.section-turbo .path-circle', {
      strokeDasharray: '0% 0% 200%', // 设置路径的虚线数组
      duration: 0.5 // 动画持续时间
    });
    timeline.to('.section-turbo .path-circle', {
      stroke: '#ffffff', // 设置路径的颜色
      duration: 0.5
    });

    timeline.to('.section-turbo .path-white .l1', {
      strokeDasharray: '11% 11% 0%', // 设置路径的虚线数组
      duration: 0.5
    });
    timeline.to(
      '.section-turbo .path-white .l2',
      { strokeDasharray: '0% 0% 11%', duration: 0.5 }, // 设置路径的虚线数组
      '<' // 表示这个动画与上一个动画同时开始
    );
    timeline.to(
      '.section-turbo .path-icon',
      { opacity: 1, duration: 0.5 }, // 设置路径图标的透明度
      '<' // 表示这个动画与上一个动画同时开始
    );

    timeline.to('.section-turbo .turbo-svg', {
      yPercent: 0, // 设置 SVG 的垂直位置
      scale: 0.8, // 设置 SVG 的缩放
      duration: 0.5
    });
    timeline.to(
      '.section-turbo .path-circle',
      { strokeWidth: 9, duration: 0.5 }, // 设置路径的描边宽度
      '<' // 表示这个动画与上一个动画同时开始
    );
    timeline.to(
      '.section-turbo .path-line',
      { strokeWidth: 5, duration: 0.5 }, // 设置路径的描边宽度
      '<' // 表示这个动画与上一个动画同时开始
    );
    timeline.to(
      '.section-turbo .path-icon',
      { strokeWidth: 9, duration: 0.5 }, // 设置路径图标的描边宽度
      '<'
    );

    timeline.to('.section-turbo .title', { opacity: 1, duration: 0.5 }); // 设置标题的透明度

    timeline.to('.section-turbo .section-wrapper', {
      yPercent: -5, // 设置容器的垂直位置
      duration: 1
    });
    timeline.fromTo(
      '.section-turbo .subtitle',
      { y: 60, opacity: 0 }, // 设置副标题的初始位置和透明度
      { y: 0, opacity: 1, duration: 1 }, // 设置副标题的最终位置和透明度
      '<' // 表示这个动画与上一个动画同时开始
    );
  });
